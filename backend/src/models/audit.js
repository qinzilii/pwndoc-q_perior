...
AuditSchema.statics.getGeneral = (isAdmin, auditId, userId) => {
    return new Promise((resolve, reject) => { 
        ...
        query.populate('reviewers', 'username firstname lastname')
        query.populate('company')
        query.select('name number auditName auditType location date date_start date_end auditor_name auditor_position daily_rate man_hour client collaborators language scope.name template customFields')
        query.exec()
        .then((row) => {
            if (!row)
                throw({fn: 'NotFound', message: 'Audit not found or Insufficient Privileges'});
